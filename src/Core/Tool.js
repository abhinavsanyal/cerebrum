...
class Tool extends Component {
  ...
  onGenerateClick = async () => {
    try {
      ...
      if (response.data.outputType) {
        console.log("outputTye:::",response.data.outputType);
        this.outputType = response.data.outputType;
      }
  ...
  render() {
    ...
    return (
      <>
        <Helmet>
          <title>{`${this.tool.title} Tool - Cerebrum`}</title>
        </Helmet>
        <Header
          ...
          <Grid>
            ...
            <Col span="6">
              <Output
                ...
                outputType={this.tool.outputType}
                code={this.code}
                language={this.language}
